-- MariaDB 개발환경 초기화 스크립트 (영어 테이블명)
USE targetDB;

-- 1. Users 테이블 (사용자 -> users)
CREATE TABLE users (
...
-- 4. Order Details 테이블 (주문상세 -> order_details)
...
-- 5. Categories 테이블 (카테고리 -> categories)
...
-- 9. Deliveries 테이블 (배송 -> deliveries)
...
SELECT 'MariaDB 개발환경 테이블 생성 완료' as message;
