...
BEGIN
    ...
    WHILE @i <= @maxUsers
    BEGIN
        INSERT INTO 사용자 (이름, 이메일, 전화번호, 주소, 생년월일, 성별, 직업)
        VALUES (
            '사용자' + CAST(@i AS NVARCHAR(10)),
            ...
            '서울시 강남구 테스트동 ' + CAST(@i AS NVARCHAR(10)) + '번지',
            DATEADD(YEAR, -(@i % 50 + 20), GETDATE()),
            CASE WHEN @i % 2 = 0 THEN '남성' ELSE '여성' END,
            CASE (@i % 5) 
                WHEN 0 THEN '개발자'
                WHEN 1 THEN '디자이너'
                WHEN 2 THEN '마케터'
    ...
    PRINT '운영환경 대용량 데이터 생성 완료';
END;
GO

PRINT 'MSSQL 운영환경 테이블 생성 완료';
GO
